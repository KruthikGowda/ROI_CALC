...
// ! Change to True For onChange outputs
// ! And False will give outputs onSubmit
...
function calculate_roi() {
  ...
  Final_Inventory_cost = Final_Inventory_cost;

  // ! Calculations of value passed into funtions parameters
  const Inventory_cost_saved_per_turn = Final_Inventory_cost * (impact_percentage * 0.01);
  ...
  // ! Anual Inventory Cost
  const Annual_Inventory_Cost = Math.floor(Final_Inventory_cost * Inventory_turns);

  // ! Testing For All The const (CONSOLE.LOGs())
  // console.log("ANNUAL REVENUE" + " " + anlRev);
  // console.log("SKU combos" + " " + No_of_SKU_combs);
  // console.log("SKU combos Range" + " " + No_of_SKU_comb_range);
  // console.log("Out of stock percentage" + " " + out_of_stock_percent);
  // console.log("impact percentage" + " " + impact_percentage);
  // console.log("Potential_out_of_stock_losses" + " " + Potential_out_of_stock_losses);
  // console.log("Scope_of_revenue_increase" + " " + Scope_of_revenue_increase);
  // console.log("Inventory_turns" + " " + Inventory_turns);
  // console.log("Inventory_cost_per_turn" + " " + Inventory_cost_per_turn);
  // console.log("Final_Inventory_cost" + " " + Final_Inventory_cost);
  // console.log("Inventory_cost_saved_per_turn" + " " + Inventory_cost_saved_per_turn);
  // console.log("Inventory_cost_can_be_saved" + " " + Inventory_cost_can_be_saved);
  // console.log("Annual_Inventory_Cost" + " " + Annual_Inventory_Cost);
  ...
  document.getElementById("ptag_2").innerHTML = `Scope of revenue increase: &nbsp;$ ${
    Scope_of_revenue_increase ? dollarUSLocale.format(Scope_of_revenue_increase) : 0
  }`;

  document.getElementById("ptag_3").innerHTML = `Final inventory cost: &nbsp;$ ${
    Final_Inventory_cost ? dollarUSLocale.format(Final_Inventory_cost.toFixed(0)) : 0
  }`;

  document.getElementById("ptag_4").innerHTML = `Inventory cost that can be saved: &nbsp;$ ${
    Inventory_cost_can_be_saved ? dollarUSLocale.format(Inventory_cost_can_be_saved) : 0
  }`;

  document.getElementById("ptag_5").innerHTML = `Annual Inventory Cost: &nbsp;$ ${
    Annual_Inventory_Cost ? dollarUSLocale.format(Annual_Inventory_Cost) : 0
  }`;
...
function Inventory_cost_can_be_saved_handler(Inventory_cost_saved_per_turn, Inventory_turns) {
  let Inventory_cost_can_be_saved = Math.round(Inventory_cost_saved_per_turn * Inventory_turns);
